function installPlayerCards(root, commands) {
  let active = null;
  function show(event) {
    const name =
      event.target instanceof Element
        ? event.target.closest("[data-bgs-player]")
        : null;
...
function createBoardThumbnail(root) {
  let preview;
  function destroy() {
    preview?.remove();
    preview = void 0;
  }
  async function render(source, size, background, omit) {
    destroy();
    ...
    if (source instanceof SVGElement) {
      const svg =
        source instanceof SVGSVGElement ? source : source.ownerSVGElement;
      if (!svg) return null;
      content = svg.cloneNode(false);
      ...
      const padding =
        source === svg ? 0 : Math.max(box.width, box.height) * 0.02;
      ...
      if (source === svg) {
        ...
        for (const defs of Array.from(svg.querySelectorAll("defs")))
          content.append(defs.cloneNode(true));
      ...
      content.style.cssText = `display:flex;flex-direction:column;gap:14px;position:relative;width:${source.getBoundingClientRect().width}px;height:${source.getBoundingClientRect().height}px;max-width:none;margin:0;transform-origin:top left;`;
    }
    if (omit)
      for (const element of Array.from(content.querySelectorAll(omit)))
        element.remove();
    let parent = preview;
    const ancestors = [];
    for (
    ...
    for (const ancestor of ancestors) {
      const context = ancestor.cloneNode(false);
...
export { createBoardThumbnail, installPlayerCards };
